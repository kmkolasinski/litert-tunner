...
class FloatMul(keras.Layer):
    """Simulates TFLite's float32 MUL op.

    The forward pass performs:
        1. Multiply in float32
    ...
    def call(
        ...
        output_float = ops.multiply(x1, x2)
...
def _build_float_mul(
    op: types.OperatorInfo,
    tensors: tuple[types.TensorInfo, ...],
) -> keras.Layer:
    """Build a FloatMul layer from parsed TFLite operator info."""
    input1_tensor = tensors[op.input_indices[0]]
    input2_tensor = tensors[op.input_indices[1]]
    ...
    return FloatMul(
        ...
        constant_input=constant_input,
        constant_input_index=constant_input_index,
        name=f"float_mul_{op.output_indices[0]}",
    )
...
def build_mul(
    ...
    """Build a MUL layer from parsed TFLite operator info."""
    input_tensor = tensors[op.input_indices[0]]
    if types.is_quantized(input_tensor):
        return _build_quantized_mul(op, tensors)
    return _build_float_mul(op, tensors)
