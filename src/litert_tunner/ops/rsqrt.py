"""RSQRT op implementation for litert_tunner."""
...
class QuantizedRsqrt(keras.Layer, types.Writable):
    """Simulates TFLite's quantized RSQRT (reciprocal square root) op.

    The forward pass performs:
        1. Dequantize INT8 input to float32
        2. Apply reciprocal square root: 1 / sqrt(input)
    ...
    def call(self, x: TensorLike) -> TensorLike:
        """Forward pass simulating quantized RSQRT."""
        # 1. Dequantize
        input_float = self.input_quant.dequantize(x)
        # 2. Reciprocal square root
        output_float = ops.rsqrt(input_float)
    ...
    def collect_write_ops(
        ...
        """Return flatbuffer write instructions for the RSQRT layer."""
...
def build_rsqrt(
    ...
    """Build a QuantizedRsqrt layer from parsed TFLite operator info."""
    ...
    if input_quant is None or output_quant is None:
        msg = "RSQRT requires quantized input and output tensors"
        raise ValueError(msg)

    return QuantizedRsqrt(
