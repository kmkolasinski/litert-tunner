"""DIV op implementation for litert_tunner.
...
class QuantizedDiv(keras.Layer, types.Writable):
    """Simulates TFLite's quantized DIV op.

    The forward pass performs:
        1. Dequantize both INT8 inputs to float32 (if quantized)
        2. Divide in float32
        3. Apply fused activation (if any)
        4. Fake-quantize output to INT8 (if quantized)

    When all quantization params are ``None``, the layer acts as a simple
    float32 division passthrough.

    Trainable parameters (when quantized): output_scale, output_zero_point.
    Frozen parameters (when quantized): input scales and zero-points.
    """

    def __init__(
        self,
        input1_scale: float | None,
        input1_zero_point: float | None,
        input2_scale: float | None,
        input2_zero_point: float | None,
    ...
    def build(self, input_shape: ShapeLike) -> None:
        ...
        if self._input1_scale is not None and self._input1_zero_point is not None:
            ...
            self.input1_quant = None

        if self._input2_scale is not None and self._input2_zero_point is not None:
    ...
    def call(
        ...
        """Forward pass simulating quantized DIV.
        ...
        # 1. Dequantize
        x1_float = self.input1_quant.dequantize(x1) if self.input1_quant is not None else x1
        x2_float = self.input2_quant.dequantize(x2) if self.input2_quant is not None else x2
        # 2. Divide
        output_float = ops.divide(x1_float, x2_float)
        # 3. Fused activation
        output_float = utils.apply_fused_activation(output_float, self._fused_activation)
        # 4. Quantize to simulated INT8 (if quantized)
    ...
    def collect_write_ops(
        ...
        """Return flatbuffer write instructions for the DIV layer."""
        quant_writes: list[types.QuantizationWriteOp] = []
        if self.input1_quant is not None:
            quant_writes.append(self.input1_quant.make_write_op(op.input_indices[0]))
        if self.input2_quant is not None:
...
class FloatDiv(keras.Layer):
    """Simulates TFLite's float32 DIV op.
...
def _build_quantized_div(
    op: types.OperatorInfo,
    tensors: tuple[types.TensorInfo, ...],
) -> keras.Layer:
    """Build a QuantizedDiv layer from parsed TFLite operator info."""
    ...
    # Detect constant inputs — works for both quantized and float32 passthrough
    constant_input: np.ndarray | None = None
    constant_input_index = -1
    if input1_quant is not None and input2_quant is not None:
        # Quantized path: use shared helper (stores as simulated INT8)
        constant_input, constant_input_index = utils.extract_constant_input(
            input1_tensor, input1_quant, input2_tensor, input2_quant
        )
    elif input1_tensor.data is not None:
        # Float32 passthrough: detect constant data directly
        constant_input = input1_tensor.data.astype(np.float32)
        constant_input_index = 0
    elif input2_tensor.data is not None:
        constant_input = input2_tensor.data.astype(np.float32)
        constant_input_index = 1

    input1_scale = float(input1_quant.scales[0]) if input1_quant is not None else None
    input1_zp = float(input1_quant.zero_points[0]) if input1_quant is not None else None
    input2_scale = float(input2_quant.scales[0]) if input2_quant is not None else None
    input2_zp = float(input2_quant.zero_points[0]) if input2_quant is not None else None
    output_scale = float(output_quant.scales[0]) if output_quant is not None else None
    output_zp = float(output_quant.zero_points[0]) if output_quant is not None else None

    return QuantizedDiv(
        input1_scale=input1_scale,
        input1_zero_point=input1_zp,
        input2_scale=input2_scale,
        input2_zero_point=input2_zp,
        output_scale=output_scale,
        output_zero_point=output_zp,
        fused_activation=fused_activation,
        constant_input=constant_input,
        constant_input_index=constant_input_index,
        name=f"quantized_div_{op.output_indices[0]}",
    )


def _build_float_div(
    op: types.OperatorInfo,
    tensors: tuple[types.TensorInfo, ...],
) -> keras.Layer:
    """Build a FloatDiv layer from parsed TFLite operator info."""
    input1_tensor = tensors[op.input_indices[0]]
    input2_tensor = tensors[op.input_indices[1]]
    ...
    return FloatDiv(
        ...
        constant_input=constant_input,
        constant_input_index=constant_input_index,
        name=f"float_div_{op.output_indices[0]}",
    )
...
def build_div(
    ...
    """Build a DIV layer from parsed TFLite operator info."""
    input_tensor = tensors[op.input_indices[0]]
    if types.is_quantized(input_tensor):
        return _build_quantized_div(op, tensors)
    return _build_float_div(op, tensors)
